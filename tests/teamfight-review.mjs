...
function setMetrics(r){
 const mtr={
  win:r.winner==='nva'?1:0, nexus:r.endReason==='NEXUS'?1:0, cap:r.endReason==='CAP'?1:0,
...
// 강화 효과: 동일 시드 짝. McNemar 기반 95% 신뢰구간(승패는 세트 단위 0/1).
function pairedCI(bwin,ewin){
 let b=0,c=0; // b: equal승·강화패, c: equal패·강화승
 for(let i=0;i<N;i++){ if(bwin[i]===0&&ewin[i]===1)c++; else if(bwin[i]===1&&ewin[i]===0)b++; }
 const d=(c-b)/N;                                   // 승률 차(강화−균형)
 const varD=(b+c - (c-b)*(c-b)/N)/(N*N);            // 불일치쌍 기반 분산
 const se=Math.sqrt(Math.max(varD,0));
 return {d:d*100, lo:(d-1.96*se)*100, hi:(d+1.96*se)*100, disc:b+c};
}

console.log(`\n=== 단위 5 통제 실험 (N=${N} 페어 시드) ===`);
console.log(`균형: 세트 A승 ${f2(eq.setWinPct)}%  | 정상(NEXUS) ${f2(eq.nexusPct)}%  상한(CAP) ${f2(eq.capPct)}%  | 사건/세트 ${f2(eq.eventsAvg)}`);
...
console.log(`\n조건별 세트 승률 (강화−균형) %p [95% CI], 그리고 행동 지표 Δ:`);
for(const r of rows){
 const ci=pairedCI(eq.winBySeed,r.winBySeed);
 const d=x=>{const v=x(r)-x(eq);return (v>=0?'+':'')+f2(v);};
 console.log(
  `${r.name.padEnd(12)}  Δ승률 ${ci.d>=0?'+':''}${f2(ci.d)}p  [${f2(ci.lo)}, ${f2(ci.hi)}]  불일치쌍 ${ci.disc}`+
