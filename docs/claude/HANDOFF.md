...
갱신: 2026-09-11 / **MINIMAP 2단계 1부(D021): 합류 이동 원인 분류 + 도착 기반 어시스트 표시 필터** 완료 · **다음 = 엔진 참가 판정에 이동 시간 실제 통합**(범위 밖으로 명시 미룸, 착수 조건 아래) · MATCH-SYS 단위 6 step 5(리캡)는 그 다음
Git: `025aedd`(D017/D018 인수인계) → `3198ce2`(단위 6 POG, D019) → `53a348e`(D019 인수인계) → `0b58edb`(POG 수정+미니맵 연속성, D020) → `a15915a`(D020 인수인계) → `3e644ba`(MINIMAP 2단계 1부, D021) → (이 문서 커밋). tag `minimap-pre-persistent` = MINIMAP-02 이전 상태(비교 영상용). 배포 없음.

## 현재 목표 — 우선순위(사용자 지시, 2026-09-11 유지)
**1순위: MINIMAP-01/02 완성.** D020에서 사건-경계 이동 연속성을 고쳤고, D021에서 "MINIMAP 2단계"(접근 도착 시각 ↔ 합류 판정 통합)에 착수했다 — 단, **표시(replay.ts) 계층에서 안전한 부분만** 이번 세션에 실제로 마쳤고, **엔진(`combat.ts`)의 참가 판정 자체를 이동 시간 기반으로 재설계하는 핵심 부분은 다음 세션으로 명시적으로 미뤘다**(이유: 결과-영향 엔진 변경은 이 프로젝트에서 항상 수천 시드 통제 실험 + McNemar CI로 검증해왔고, 한 세션에 새 확률 채널 설계·구현·검증까지 책임 있게 마칠 수 없다고 판단했다 — DECISIONS D021 "범위 결정" 절 참조). **모바일/탭전환이 실기기로 확인되기 전까지, 그리고 엔진 통합이 D014~D017급 검증을 거치기 전까지 MINIMAP을 완료로 표시하지 않는다.**
**2순위: MATCH-SYS 단위 6 step 5(리캡 근거 사건화).** MINIMAP 완성 다음으로 미룬다(사용자 지시, D020부터 유지).
...
- ⬜ **엔진 참가 판정에 이동 시간 실제 통합 ← 다음 첫 행동**(핵심, 범위 밖으로 미룸) — 아래 절 참조.
- ⬜ **모바일/탭전환 실기기 확인** — 3연속 세션(D018·D020·D021) 도구 제약(CDP 스크린샷 1568px 고정)으로 미확인.
...
## 아직 실패하거나 미검증인 것
- **엔진 참가 판정에 이동 시간 미통합**(D018→D020→D021 모두 미착수, 사용자 지시의 핵심): `showDelay`/`nextOwnEvent`/`arrivalEst`는 전부 화면 렌더·이동 계획·표시 필터일 뿐, `combat.participants`가 여전히 authoritative. **이게 이번 세션 이후 최우선 미완료 항목** — 아래 "다음 첫 행동" 참조.
- **killer 표시 타이밍 불일치(D021 발견, 미수정)**: 처치 시각에 killer가 아직 'fight' 상태가 아닌 사례 150시드 2,504개 처치 중 464건(18.5%). 어시스트 필터(D021)와 달리 이번 요청 범위 밖이라 손대지 않음.
- **오브젝트 이벤트의 어시스트 필터 미확인**: 코드 경로는 공통이라 자동 적용되지만, `resolveObjective`의 `arriveP`가 이동 시간과 무관해 갱킹/한타보다 화면-판정 괴리가 클 수 있다 — 별도 표본 미확인.
- **모바일 뷰포트 실기기 미확인**(D018·D020·D021 3연속 동일 제약): `resize_window(390×844)` + 새로고침 후에도 CDP 스크린샷이 1568px 고정 — 코드상 `@media(max-width:820px)` 반응형 규칙은 정적 확인만. **실제 모바일 기기나 브라우저 개발자 도구를 직접 조작할 수 있는 환경에서 확인 전까지 이 항목을 완료로 표시하지 않는다.**
...
- **비교 영상 미첨부**(D018·D020·D021): 세 세션 모두 자동화 Chrome이 GIF를 녹화·내보내기(다운로드)까지는 성공했으나 이 파일시스템에서 접근 불가 — Chrome 확장의 다운로드 경로가 이 세션 파일시스템과 분리된 것으로 보임(원인 미확정, 3연속 재현). 대신 D021은 시각 찍힌 연속 스크린샷 4장을 남김(아래 재현 절차). 변경 전 화면 = `git checkout minimap-pre-persistent` 필요(현재 코드로는 재현 불가).
...
**D020~D021로 이미 끝난 것**(재구현 금지): 10명 전체 지속 이동(D016), 사건 경계 위치 연속성(D020, `nextOwnEvent`), 합류 이동 원인 분류(D021), 미래 사건 참조 경계 확인(D021), 표시 계층의 어시스트 도착 필터(D021). 이 부분들을 다시 만들지 말고 그대로 둔다.

**남은 핵심**: 사용자가 요구한 "도착과 참여의 공통 계약"(3~4번 항목)의 전체 버전은 `combat.ts`(`resolveObjective`/`resolveTeamfight`)의 참가 판정 자체가 이동 시간을 실제로 반영하게 만드는 것이다. D021은 이 부분을 **의도적으로 범위 밖에 뒀다** — 결과에 영향을 주는 엔진 변경이라 이 프로젝트의 기존 검증 기준(D014~D017의 수천 시드 McNemar CI)을 만족해야 "완료"라 부를 수 있는데, 그 검증까지 한 세션에 책임 있게 마칠 자신이 없었기 때문이다(DECISIONS D021 "범위 결정" 절에 이유 상세).

### 미완료 조건 (이 중 하나라도 남아 있으면 "MINIMAP 완성"을 계속 최우선으로 선택할 것)
1. **엔진 참가 판정에 이동 시간 미통합** — 아래 "정확한 착수 절차" 참조. 유일하게 남은, 새로 설계해야 하는 큰 덩어리.
2. **모바일 뷰포트 실기기 확인 안 됨** — 3세션(D018·D020·D021) 연속 브라우저 자동화 도구의 CDP 스크린샷이 1568px에 고정되어 실측 불가.
3. **탭 비활성/복귀 정확한 재현 안 됨** — 자동화 중 정지는 관찰했으나 실사용자 탭 전환과 같은 신호인지 불확실.
4. **비교 영상(변경 전 대비) 미첨부** — GIF는 세 세션 다 녹화·내보내기 성공했으나 파일시스템에서 회수 불가(원인 미확정).
5. (부가) killer 표시 타이밍 불일치(18.5%, D021 발견)·오브젝트 이벤트 어시스트 필터 별도 확인 — 위 1번을 시작하기 전에 먼저 처리해도 되고, 1번과 함께 다뤄도 된다.

### 정확한 착수 절차 — 엔진 참가 판정에 이동 시간 실제 통합 (별도 체크포인트, 결과-영향 변경)
1. **먼저 확인**(재구현 방지): `lib/simulation/replay.ts`의 `arrivalEst`·`nextOwnEvent`·`travel.causes`(D021)와 `showDelay`(D018)가 이미 무엇을 계산해 두었는지 다시 읽는다. 이 데이터(도착 추정 시각, 원인 분류)를 **입력으로 재사용**하는 것이지, 이동 시뮬레이션을 다시 만드는 게 아니다.
2. **설계를 먼저 문서화**(코드 전에, DECISIONS.md 초안): `resolveObjective`의 `arriveP`, `resolveTeamfight`의 참가자 결정(현재는 전원 참가, 인원차 `numAdvFav`만 반영)이 이동 시간 기반 도착 가능성을 **정확히 어떻게** 반영할지 구체적 산식으로 적는다. "독립 확률을 두 번 적용하지 않는다"는 원칙 — 예: 이동이 이미 물리적으로 불가능하다고 판정된 참가자는 능력치 기반 확률 롤 자체를 건너뛰고 확정적으로 불참 처리, 물리적으로 가능한 범위 안에서는 기존 확률만 사용(추가 확률 채널 없음).
3. **combat.ts는 replay.ts의 WALK 그래프를 몰라야 한다는 기존 경계를 어떻게 다룰지 결정**: 옵션 (a) combat.ts가 이미 가진 추상 `Region`/`REGION_DIST`(단위 5, D015/D018)를 참가 판정에 더 깊이 활용 — WALK 그래프 좌표는 몰라도 되고 이미 존재하는 지역-이동시간표만 쓰면 replay.ts 의존을 안 만든다. 옵션 (b) replay.ts의 정밀 도착 추정을 combat.ts에 콜백/사전계산으로 전달 — 아키텍처 결합이 커진다. (a)를 먼저 검토할 것(기존 경계를 덜 깨뜨림).
4. **결과가 바뀌는 부분을 명시 기록**: 어느 유형의 사건에서 어떤 참가자가 새로 불참 처리되는지(또는 그 반대) 표로 남긴다. `combat.test`/`teamfight-review.mjs`로 이전 D017/D019/D020/D021 측정치(승률·NEXUS/CAP 비율·참가율·딜러 생존 등)와 나란히 재측정 — 방향과 크기를 있는 그대로 both 보고한다(개선이라 주장하지 않는다).
5. **완료 조건**: (a) 설계 문서화 완료, (b) 엔진 참가자 결정 로직 변경 + 결과 변화 표, (c) `replay.ts` 연결(D021의 `arrivalEst`/`nextOwnEvent` 재사용), (d) `combat.test`/`replay.test`/`teamfight-review` 갱신 + PASS, (e) tsc/build 0, (f) **McNemar CI 등 D014~D017급 통계 검증**(수천 시드) — 이게 없으면 "완료"라 쓰지 않는다, (g) PC+모바일 브라우저로 새 합류 판정이 만든 화면(예: 전에는 화면에 보이던 참가자가 이제 `notJoined`로 빠지는 경우)을 실제로 관찰.
