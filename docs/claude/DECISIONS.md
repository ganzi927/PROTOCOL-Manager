...
## D030 — F25 연습 모드: simulateSet의 순수성을 그대로 격리 경계로 쓴다

2026-09-16. F25(첫 경기 튜토리얼과 연습 모드) 완료 조건: "샌드박스에서는 두 팀과 조합을 고정하고 같은 조건을 다시 비교할 수 있게 한다", "연습 결과가 본 시즌 승패·성장에 섞이지 않는다."

### 문제
`Game` 타입에 `sandbox?:boolean` 필드가 이미 선언돼 있었지만 코드 어디에서도 읽거나 쓰지 않는 죽은 필드였다 — 기능 자체가 설계만 되고 구현된 적이 없었다.

### 선택
1. **`simulateSet`이 이미 순수 함수라는 사실을 그대로 격리 경계로 삼았다.** 감사해 보니 `simulateSet(g,m,directive)`은 `g.players`/`g.teams`를 읽기만 하고 아무것도 쓰지 않는다 — 실제로 번아웃·폼·숙련도·POG·스카우팅 표본·훈련 이력·`g.history`·팀 승패를 갱신하는 건 별도 함수 `finishMatch()`이며, 연습 모드는 그 함수를 절대 호출하지 않는다. 그래서 "본 시즌에 안 섞인다"는 조건 문(if)으로 막은 게 아니라 애초에 그 코드 경로 자체가 없어서 구조적으로 보장된다 — `tests/sandbox.test.mjs`가 이걸 "실행 전후 히스토리·선수 상태·팀 전적 바이트 단위 동일"로 직접 증명한다.
2. **결과 저장은 `Game.sandboxLast`(마지막 1건만) — 새 API·별도 스토리지 없음.** 일반 명령(`applyCommand`)에 `case 'sandboxMatch'`를 추가해 기존 저장·동시성 인프라(F24에서 확인한 그 인프라)를 그대로 썼다. "본 시즌에 안 섞인다"와 "그래도 저장은 된다"가 모순이 아닌 이유는 (1)에서 설명한 구조적 분리 때문이다.
3. **재현성은 시드를 클라이언트가 명시적으로 넘기는 방식으로.** `applyCommand`를 명령 입력에 대해 순수하게 유지하려고 명령 처리 중 `Math.random()`을 쓰지 않았다 — "매치 실행"은 클라이언트가 새 난수 시드를 생성해 넘기고, "같은 조건 다시 보기"는 저장된 시드를 그대로 재전송한다. `m.id`에 시드를 포함시켜(`sandbox-${a}-${b}-${seed}`) 실제 승부/골드/중계 RNG 스트림이 시드마다 독립적으로 갈리게 했다(같은 시드·같은 매치업은 항상 같은 결과 — 브라우저로 직접 재확인: 같은 시드 재생 시 클럭·이벤트·골드가 정확히 동일).
4. **화면은 새로 안 만들고 기존 `ReplayTheater`를 그대로 재사용했다.** `SetResult`가 이미 `ReplayTheater`가 요구하는 `endReason/events/lineupA/lineupB/draft` 필드를 전부 갖고 있어 어댑터 코드 없이 바로 넘길 수 있었다 — 미니맵·중계·KDA까지 실제 경기와 완전히 동일한 품질로 공짜로 따라왔다.
5. **v1 범위: 팀 매치업만 고정, 픽/밴은 자동(AI).** "조합을 고정"의 완전한 의미(사용자가 직접 특정 챔피언 조합을 지정)는 이번에 안 했다 — 두 팀을 고르면 `pickDraft`의 기존 자동 드래프트가 동작한다. 같은 시드면 같은 조합이 나오므로 "같은 조건 재비교"는 되지만, "내가 원하는 특정 조합끼리 비교"는 다음 조각.

### 이유
"연습 결과가 본 시즌에 안 섞인다"를 검증 가능한 방식으로 보장하려면 조건문이 아니라 코드 경로 자체의 부재가 훨씬 강한 증거다 — 실수로 나중에 `if(!g.sandbox)` 같은 분기를 빼먹어도 안전한 설계. `finishMatch`를 호출하는 유일한 경로(`case 'continue'`)를 건드리지 않았다는 사실 하나로 이 보장이 성립한다.

### 영향
- 신규: `lib/game.ts`의 `Game.sandboxLast`, `case 'sandboxMatch'`, `tests/sandbox.test.mjs`(4섹션).
- 변경: `app/manager.tsx`(연습 모드 nav·뷰, `ReplayTheater` 재사용).
- 회귀 위험 검토: 기존 `case`는 전혀 안 건드림(새 case 추가만). `simulateSet` 자체도 무변경. `management.test.mjs`·`engine.test.mjs` 재확인 PASS. 브라우저 실측: 매치 실행→미니맵·중계 실시간 재생 확인→"같은 조건 다시 보기"로 완전히 동일한 진행(클럭 02:38, 이벤트, 골드 3.7k vs 3.3k 전부 일치) 재확인, 콘솔 에러 없음.

### 대안(기각)
- **사용자가 직접 픽/밴하는 샌드박스 드래프트 UI.** 실제 드래프트 화면(`LiveDraft`)을 연습 모드용으로 분기하는 건 상당한 UI 작업이라 이번 범위를 넘는다고 판단 — v1은 팀 고정+자동 드래프트로 시작.
- **연습 결과를 아예 저장하지 않음(순수 클라이언트 계산).** 이 앱 전체가 "모든 확정 행동은 서버에 저장됩니다"를 지켜 왔는데 연습 모드만 예외로 두면 일관성이 깨진다 — 저장은 하되 시즌 로직과 코드 경로 자체를 분리하는 쪽을 택함.
...
## D032 — F27 실제 조합 통계: 외부 서비스를 흉내 내지 않고 이 리그 자체의 g.scout을 "실측"으로 쓴다

2026-09-16. F27(실제 조합 통계와 메타 데이터 연결) 완료 조건: "관측 승률/모델 조합 전망/우리 팀 전망이 분리된다. 데이터 실패·오래된 패치·작은 표본에 폴백이 있다. 외부 값을 적용한 새 규칙 버전과 기존 리플레이가 분리된다. 특정 사이트의 화면 숫자가 곧 이 게임의 승률이라는 가정은 금지한다." 명령 자체가 "외부 서비스를 화면에서 흉내 내기 전에 사용할 수 있는 데이터·접근 방식·출처·조건을 확인하라"고 먼저 요구한다.

### 문제
이 게임은 140종 전부 가상 챔피언·가상 팀으로 이뤄진 완전히 자체 완결된 시뮬레이션이다 — 실제 롤 승률 데이터를 연결할 대상 자체가 존재하지 않는다(op.gg류 실제 서비스가 이 가상 챔피언들의 승률을 갖고 있을 리 없다). `lib/balance/composition.ts`는 이미 파일 상단 주석에 "검증된 외부 랭크/대회 데이터는 확보하지 않았다... 전부 명시적 게임 설계값이며 실제 승률의 복제물이 아니다"라고 스스로 밝히고 있었고, `composition-panel.tsx`도 이미 "게임 설계 지표, 실측 승률 아님"이라고 라벨을 달고 있어 — 우려했던 "태그 기반 점수를 실측 승률로 이름만 바꾸는" 위반은 애초에 없었다(감사로 확인).

### 선택
1. **"실제 데이터"를 이 게임 자신의 완료된 경기 기록으로 정의했다 — 외부 서비스를 흉내 내지 않는다.** `g.scout`(F16)을 감사한 결과 사용자 팀뿐 아니라 매 경기(`finishMatch`) 양 팀 관점으로 실제 픽·승패가 이미 쌓이고 있었다(팀당 최근 SCOUT_CAP=15세트, 리그 전체 10+팀). 새 저장소·새 API 없이 이 실제 데이터만 읽어 챔피언·역할별 관측 승률을 집계하는 `lib/balance/observed.ts`를 신설했다 — 순수 읽기 전용, `g.scout`/`g.replays`를 절대 쓰지 않는다.
2. **표본 임계값(MIN_SAMPLE=6)은 실측으로 정했다.** 3시즌(739커맨드) scratch 시뮬레이션에서 관측된 챔피언-역할 조합 121개 중 97개(80%)가 6표본 이상을 확보했다 — 감으로 정하지 않고 F18/F21과 같은 방식으로 실제 분포를 먼저 찍어보고 정했다.
3. **5인 완전 조합 단위 집계 함수는 아예 만들지 않았다.** 이 표본 규모(팀당 최근 15세트)에서 정확히 같은 5인 조합이 두 번 이상 나올 확률은 사실상 0에 가깝다 — "희소한 5인 조합은 샘플 부족으로 표시"를 조건문 폴백이 아니라 그 집계 함수 자체의 부재로 구조적으로 지켰다(F25/F26에서 반복한 것과 같은 패턴: 코드 경로가 없으면 실수로 숫자를 지어낼 수 없다). 2인 시너지·상대 조합 대응·선수 개인 실력도 같은 이유로 v1에서 분리해 만들지 않았다("무리하게 같은 원인으로 합치지 않는다"는 완료 조건 문구 그대로).
4. **관측 승률/모델 조합 전망/우리 팀 전망을 화면에서 명시적으로 분리했다.** `CompositionPanel`에 "관측 승률(이 리그 실제 경기 기록)" 절을 새로 추가하고, 기존 설계값 절 라벨을 "모델 조합 전망(게임 설계 지표, 실측 승률 아님)"으로 명확히 했다. "우리 팀 전망"(로스터 실제 능력치 기반 팀 전력 표시)은 이미 PREP 화면에 별도 카드로 존재해 손대지 않았다 — 세 값이 이미 서로 다른 화면 요소였다.
5. **표본 부족(폴백)은 null을 반환하고 UI가 "표본 부족"으로 표시한다.** 숫자를 억지로 만들지 않는다 — `winRate: total>=MIN_SAMPLE ? wins/total : null`. 새 게임(경기 0회)은 모든 챔피언이 표본 부족으로 시작하는 게 정직한 기본값이다(데이터 실패 시 폴백이 곧 "설계값 모드로 정상 동작"이라는 완료 조건 그 자체).
6. **"패치" 분리는 있는 척 지어내지 않았다.** 이 게임엔 실행 중 값이 바뀌는 실시간 밸런스 패치 시스템이 없다(`CompProfile`은 상수). 대신 F26의 `RULE_VERSION`/시즌 번호를 관측치에 스탬프로 남겨, 나중에 실제 패치 시스템이 생기면 그 스탬프로 구세대 관측치를 구분할 수 있게만 해뒀다 — 존재하지 않는 개념을 UI에 전시하지 않았다.
7. **`SetScout`에 `stage?:'REGULAR'|'PLAYOFF'|'INTERNATIONAL'`를 선택 필드로 추가했다.** "대회/일반 경기 맥락" 요구를 위해 `finishMatch` 시점의 `g.stage`를 그대로 찍는다 — 기존 필드는 전혀 안 건드리고 새 선택 필드만 추가해 구세이브 호환성이 자동으로 유지된다. v1 UI는 이 필드를 아직 노출하지 않는다(관측 승률 집계에도 아직 반영 안 함) — 다음 조각으로 명시적으로 남긴다.

### 이유
"흉내 내기 전에 확인하라"는 명령을 문자 그대로 따르면, 이 게임에 대해 실제로 존재하는 유일한 "실측 데이터"는 이 게임 자신이 실제로 시뮬레이션한 경기 결과뿐이다. 외부 API도, 오프라인 fixture로 가져올 실제 서비스도 없는 게 이 프로젝트의 정직한 현실이라, 그 현실을 감추고 가짜 "외부 연동" 껍데기를 만드는 대신 실제로 있는 내부 데이터(g.scout)를 "관측치"로 격상시켜 쓰는 쪽을 택했다. 이렇게 하면 F27이 요구하는 모든 완료 조건(폴백, 분리, 표본 부족 정직 표시)을 가짜 데이터 없이 그대로 만족시킬 수 있다.

### 영향
- 신규: `lib/balance/observed.ts`(`MIN_SAMPLE`, `observedChampStat`, `observedTeamStats`), `tests/observed-stats.test.mjs`(5섹션).
- 변경: `lib/game.ts`의 `SetScout.stage?`(선택 필드) + `finishMatch`의 적립 시점에 `stage:g.stage` 한 줄, `app/composition-panel.tsx`(관측 승률 절 추가, 기존 라벨 명확화), `app/manager.tsx`(두 `CompositionPanel` 호출부에 `g` prop 전달), `app/globals.css`(`.comp-observed`).
- 회귀 위험 검토: `finishMatch`의 스카우팅 적립 루프에 필드 하나 추가(값 계산·순서 변경 없음) — `tests/scout.test.mjs`(기존 스위트, `SetScout` 형태에 의존) 재확인 PASS. tsc·`npm run build`·eslint(변경 전후 동일 18 problems — 새 에러/경고 0개)·`management.test.mjs`·`engine.test.mjs` 전부 재확인 PASS. 브라우저 실측: 실제 드래프트에서 두 챔피언을 픽해 "관측 승률" 절이 정확히 "표본 부족 (N전)"으로 렌더링됨을 확인(이 세이브는 시즌 초반이라 아직 MIN_SAMPLE을 넘긴 챔피언이 없음 — 충분한 표본일 때의 실제 퍼센트 경로는 `tests/observed-stats.test.mjs` 섹션 2가 g.scout 재집계와 바이트 단위로 교차 검증), 콘솔 에러 없음.

### 대안(기각)
- **정말로 외부 API/정적 파일에서 "실제 롤 데이터"를 가져와 흉내 낸다.** 이 게임의 챔피언은 전부 가상이라 실제 롤 승률이 애초에 대응되지 않는다 — 명령이 명시적으로 금지한 "외부 서비스를 화면에서 흉내 내는" 행위 그 자체가 된다. 확실히 기각.
- **CompProfile(설계값)을 그대로 "관측 승률"이라는 이름으로 재노출.** 명령이 명시적으로 금지한 "현재 코드의 태그 기반 점수를 실측 승률로 이름만 바꾸지 마라"에 정면으로 위배 — 확실히 기각.
- **5인 조합·2인 시너지까지 억지로 g.scout에서 뽑아낸다.** 표본이 지나치게 희소해 신뢰구간 없는 숫자를 그대로 노출하면 오히려 "지어낸 정밀도"가 된다 — v1 범위 밖으로 명시적으로 남김.

관련: F16(g.scout 데이터 출처 그 자체) / F26(ENGINE_VERSION/RULE_VERSION 재사용, 코드 경로 부재로 구조적 보장을 만드는 동일 패턴) / F18·F21(표본 임계값·가중치를 실측으로 정하는 선례) / composition.ts 파일 상단 주석(F05/F06, "설계값이지 실측 아니다"라는 기존 원칙을 F27이 그대로 계승).

## D033 — 저장 슬롯 초기화: 명령/revision 경로가 아니라 별도 DELETE로 분리한다

2026-09-16. 사용자 요청("커리어를 초기화하는 방법은 없어?" → "만들어줘"). Development Orders에 없는 항목이라 F-번호 대신 `docs/claude/BACKLOG.md`에 **SAVE-02**로 기록.

### 문제
`설정 및 저장` 화면은 이미 "새 게임은 상단의 다른 커리어 슬롯에서 시작하세요. 현재 기록은 유지됩니다"라고 안내하고 있었다 — 즉 지금 쓰는 슬롯 자체를 완전히 밀고 새로 시작하는 기능이 없었다(감사로 확인). 서버(`app/api/game/route.ts`)도 `command.type==='create'`는 그 슬롯에 행(`row`)이 아직 없을 때만 허용하고, 이미 있으면 `applyCommand`의 `default` 분기(`알 수 없는 작업입니다`)로 떨어져 에러가 난다 — DB에 슬롯 행을 지우는 경로 자체가 없었다.

### 선택
1. **기존 명령/revision 경로(`act`, `POST /api/game`)를 재사용하지 않고 새 `DELETE /api/game?slot=N`을 만들었다.** 이 작업은 행 자체를 지우는 것이라 revision 기반 낙관적 동시성·commandId 영수증 재시도 개념이 적용될 대상(row)이 애초에 사라진다 — 억지로 기존 명령 파이프라인에 끼워 넣으면 "revision이 안 맞아 재시도했는데 사실은 이미 지워짐" 같은 혼란스러운 경합 상태를 만들 위험이 있어, 처음부터 독립된 엔드포인트로 분리했다.
2. **복원 지점(backups)까지 함께 지운다 — 가져오기 덮어쓰기와 의도적으로 다르게 만들었다.** F24의 가져오기는 "덮어쓰기 직전 상태가 자동으로 백업에 남아 되돌릴 수 있음"이 핵심 안전장치였다. 슬롯 초기화는 사용자가 명시적으로 "완전히 지우고 싶다"고 요청한 기능이라, 백업까지 남기면 "초기화했는데 왜 예전 기록이 복원 지점에 남아있냐"는 혼란을 만든다 — `DELETE FROM careers WHERE owner=? AND slot=?` 한 줄로 행 전체(state+backups+receipts)를 지운다. 확인 다이얼로그 문구에 이 차이("가져오기 덮어쓰기와 달리 복원 지점도 함께 지워집니다")를 명시해 사용자가 오해하지 않게 했다.
3. **클라이언트에 두 번째 확인 다이얼로그를 별도로 만들었다.** 기존 `confirm` 상태는 `command:Command`를 담아 `act()`로 보내는 계약이라, DELETE라는 다른 HTTP 메서드·다른 응답 형태를 억지로 그 타입에 끼워 맞추면 `Command` 타입에 존재하지 않는 가짜 명령을 만들어야 했다 — 대신 `confirmReset:boolean` 상태와 전용 `resetSlot()` 함수로 완전히 분리했다.
4. **위험 구역(danger zone)을 시각적으로 분리했다.** `설정 및 저장`의 다른 패널과 다르게 붉은 톤 배경(`destructive` 버튼, `.danger-zone` 스타일)을 줘서 실수로 다른 버튼과 헷갈리지 않게 했다.

### 이유
"삭제는 되돌릴 수 없다"는 이 앱의 다른 모든 파괴적 동작(가져오기 덮어쓰기조차 백업으로 되돌릴 수 있음)보다 한 단계 더 강한 동작이라, 기존 되돌리기 가능한 경로들과 코드·UI 양쪽에서 명확히 구분해야 사용자가 실수로 "그냥 덮어쓰기겠지" 하고 누르는 사고를 막을 수 있다.

### 영향
- 신규: `app/api/game/route.ts`의 `DELETE` 핸들러, `app/manager.tsx`의 `confirmReset` 상태·`resetSlot()`·"위험 구역" 패널·두 번째 `Dialog`, `app/globals.css`의 `.danger-zone`.
- 회귀 위험 검토: `applyCommand`·엔진 코드 무변경(이 기능은 그 경로를 아예 안 씀) — `management.test.mjs` 재확인 PASS, tsc·`npm run build`·eslint(변경 전후 14 problems로 동일, 새 이슈 0개) 전부 PASS. **브라우저 실측(실제 DB 대상)**: 커리어 3(기존에 실제로 존재하던 슬롯)에서 "이 슬롯 초기화" 클릭 → 확인 다이얼로그 문구 확인 → 초기화 확정 → 성공 토스트+"새로운 시작" 팀 선택 화면으로 정확히 전환 확인 → `GET /api/game?slot=3`가 `game:null`을 반환함을 직접 확인(클라이언트 상태만 지운 게 아니라 DB 행 자체가 삭제됐음을 서버 응답으로 증명) → `GET /api/game?slot=1`(별도 슬롯)이 전혀 영향받지 않았음을 직접 확인(슬롯 격리). 콘솔 에러 없음.

### 대안(기각)
- **기존 `importSave`처럼 명령/revision 경로에 새 `resetCareer` 명령을 추가.** revision이 아직 안 맞는 상태에서 재시도가 들어오면 "지워진 행을 다시 찾으려는" 경합이 생길 수 있어, 행 자체를 없애는 동작은 그 경로 밖으로 완전히 분리하는 게 더 안전하다고 판단.
- **초기화해도 복원 지점은 남긴다.** F24와의 일관성은 있지만, "초기화"라는 사용자 의도(완전히 지우고 싶음)와 어긋나 혼란을 유발할 여지가 더 크다고 판단해 기각.

관련: F24(D029, 가져오기 덮어쓰기·복원 지점의 기존 안전장치와 이번 기능의 의도적 차이) / `설정 및 저장` 화면의 기존 문구("새 게임은 다른 슬롯에서 시작하세요" — 이번 기능 추가 전까지의 유일한 대안이었음, BACKLOG.md SAVE-02 참조).

## D034 — 실제 로스터·사진 적용: 있는 것을 감사하고, 검증 못 한 건 검증 못 했다고 남긴다

2026-09-16. 사용자 요청: "현재 게임에 실제 팀·소속 선수·프로필 사진·게임용 능력치를 적용해줘." 기준일 2026-09-16, 범위는 LCK 우선(국제팀은 공통 구조만). 상세 산출물은 `docs/claude/REAL_ROSTER_AUDIT.md`(감사)·`docs/claude/PLAYER_RATING_MODEL.md`(평가 모델 설계)·`docs/claude/DATA_UPDATE_GUIDE.md`(갱신 절차).

### 문제
감사 결과 이 요청의 절반은 **이미 구현돼 있었다** — `lib/rosters.ts`가 진작부터 LCK 10개 팀·국제 13개 팀 전체에 실제 선수 핸들/실명을 매핑하고 있었고(`LCK_ROSTER`/`INTL_ROSTER`), `lib/game.ts`가 선발 5인에 그 이름을 오버레이하고 있었다. 다만: (a) 이 매핑이 **배열 위치 기반**(`[handle,realName][ROLES 순서]`)이라 role 필드가 없어 사용자가 명시적으로 경고한 "배열 순서나 닉네임 문자열만으로 선수 기록을 연결" 위험에 그대로 노출돼 있었다, (b) 출처·확인일·등록 상태(선발/벤치/비활성) 필드가 전혀 없어 "확인된 것"과 "추정"을 구분할 수 없었다, (c) 파일 자체 주석이 "best-effort approximation as of early 2026"이라고 자백하고 있었다 — 오늘(9월) 기준으로 최신인지 검증된 적이 없었다, (d) 선수 사진은 아예 없었다, (e) 실제 경기 기록 기반 능력치는 전혀 없었다(전부 팀 `base` 전력값에서 유도된 합성 스탯).

### 선택
1. **배열 위치 매핑을 role 키 매핑으로 바꿨다.** `RosterPlayer{handle,realName,role,status,photo?,note?}` + `TeamRosterEntry{players,sourceUrl?,confirmedAt?}`로 재구성(LCK·INTL 공통 스키마 — 사용자 지시 "해외 팀은 공통 구조만 먼저"). `lib/game.ts`의 두 소비 지점(국내 `newGame`, 해외 `upgradeGame` 백필)을 role로 찾도록 수정 — `tests/real-roster.test.mjs`로 role 매칭이 실제 정확한지 직접 검증.
2. **1차 출처 검증을 실제로 시도했고, 실제로 막혔다는 사실을 숨기지 않았다.** 서브에이전트 2개(로스터 검증용, 사진 URL용)를 병렬로 띄워 Liquipedia·LCK 공식 사이트·팀 공식 자료에 접근을 시도했으나 **이 환경의 WebFetch가 모든 시도에서 403/429/402로 차단됐다**(로스터 조사 12회 시도 전부 실패, 사진 조사도 동일). 검색 엔진 요약(WebSearch)만 접근 가능했는데, 이건 "실제로 열어 확인"이 아니다 — 사용자가 명시적으로 금지한 것("실제로 열어 확인하지 않은 출처를 검증 완료로 표시하지 마")과 정확히 일치하는 상황이라, **검색 요약으로 얻은 단서(T1 Gumayusi→HLE 이적설, DRX 개명설 등 다수)를 코드에 반영하지 않았다** — 반영했으면 미검증 정보를 확정 정보로 저장하는 것이었다. 대신 `REAL_ROSTER_AUDIT.md`에 전부 "미검증" 표로 남기고, 서로 충돌하는 단서(KT Rolster ADC/SUP, "Taeyoon"이 두 팀에 동시 등장, Gen.G Duro 실명)는 별도 검토 목록으로 뺐다.
3. **사진은 Liquipedia 대신 Wikimedia Commons로 우회해 일부 확보했다.** Liquipedia API도 429로 막혔지만, Wikipedia의 `pageimages` API(Commons 이미지, 기계 판독 가능한 라이선스 태그 보유)는 동작했다 — LCK 선발 50명 중 10명, 팀 로고 10개 중 4개를 실제 API 응답에서 확인된 URL로만 채웠다(추측 URL 없음). 나머지는 비워 뒀다 — `app/manager.tsx`의 기존 `Emblem`/`ChampTile` 컴포넌트가 이미 쓰던 "모노그램 먼저 깔고 이미지는 `onError`로 숨김" 기법을 그대로 재사용하는 `PlayerFace` 컴포넌트를 신설해, 사진이 없는 40명(선수)·6개(팀)는 조용히 이니셜 아바타로 폴백한다(깨진 이미지 없음).
4. **사진은 재호스팅하지 않고 원본 서버 직링크만 쓴다.** `PLAYER_PHOTO`/`TEAM_LOGO`는 URL만 저장하고, 렌더 시점에 조회한다 — `Player`/`Game` 저장 상태 어디에도 사진 데이터가 안 들어간다(`tests/real-roster.test.mjs`로 직접 검증). 이건 사용자가 명시한 "무단 재호스팅 금지"와 "저장 마이그레이션 위험 없음"을 동시에 만족시킨다 — 사진을 나중에 더 채워도 기존 세이브에 영향이 전혀 없다.
5. **"게임용 능력치"는 설계만 하고 실제 데이터를 채우지 않았다.** `lib/players/rating-model.ts`(원자료→전처리→게임 평가 3계층, `Player`/`Game`에 아직 연결 안 함)를 만들어 사용자가 요구한 분리(원자료/보정 지표/게임용 능력치/모델 버전/신뢰도)를 구조로 준비했지만, 실제 경기별 통계를 단 1건도 수집하지 못한 상태(같은 WebFetch 차단)에서 "진짜처럼 보이는" 숫자를 만들어 넣는 건 사용자가 명시적으로 금지한 "자료가 없는 값에 임의로 높은 점수를 주는" 행위가 된다 — 그래서 채우지 않았다. `app/manager.tsx`의 능력치 표시에 "게임 내 평가 — 공식 능력치 아님, 현재는 팀 전력 기반 설계값입니다" 라벨을 추가해 지금 상태를 화면에서도 정직하게 밝혔다.
6. **기존 커리어는 자동으로 안 바뀐다 — 우연이 아니라 기존 구조 그대로.** `LCK_ROSTER` 변경은 `newGame()`이 처음 실행될 때만 반영되고, 이미 생성된 `Player` 객체를 다시 순회해 덮어쓰는 코드 경로가 원래 없었다(감사로 확인, 새로 만들지 않음) — `tests/real-roster.test.mjs`가 `upgradeGame` 왕복 후에도 이름이 그대로임을 직접 증명한다.

### 이유
"실제 데이터를 적용해줘"라는 요청을 문자 그대로 수행하려면 실제 1차 출처 접근이 필요한데, 그 도구 자체가 이 환경에서 막혀 있었다. 이 제약을 숨기고 검색 요약을 "확인된 데이터"인 척 코드에 채워 넣는 것은 사용자가 이 요청에서 가장 여러 번 강조한 원칙("확인 완료로 표시하지 마라", "충돌은 별도 검토 목록에 남겨라", "추정으로 채우지 마라")을 정면으로 어기는 일이다. 대신 (a) 이미 있던 인프라의 진짜 설계 결함(배열 위치 매핑, 출처 필드 없음)을 실제로 고치고, (b) 확보 가능했던 진짜 데이터(Wikimedia 사진 10+4개)는 실제로 적용하고, (c) 확보하지 못한 것은 검토 목록과 문서로 정직하게 남기는 쪽을 택했다.

### 영향
- 신규: `lib/players/rating-model.ts`, `tests/real-roster.test.mjs`(6섹션), `docs/claude/REAL_ROSTER_AUDIT.md`·`PLAYER_RATING_MODEL.md`·`DATA_UPDATE_GUIDE.md`.
- 변경: `lib/rosters.ts`(전면 재구성, 기존 실명 데이터는 보존), `lib/game.ts`(두 로스터 소비 지점을 role 키 매칭으로, `PLAYER_PHOTO` 재수출), `app/manager.tsx`(`PlayerFace` 컴포넌트, 두 `player-monogram` 사용처에 적용, 능력치 라벨 추가), `app/globals.css`(`.player-monogram`/`.pm-mono` 오버레이, `.stat-disclaimer`).
- 회귀 위험 검토: `newGame()`/`upgradeGame()`(거의 모든 테스트가 거치는 핵심 경로) 변경 — `tests/management.test.mjs`·`tests/engine.test.mjs`(3커리어·1106+ 세트) 재확인 PASS. tsc·`npm run build` PASS. eslint는 새 파일(`rosters.ts`/`rating-model.ts`) 0개 이슈, 기존 파일(`game.ts`) 델타 0, `manager.tsx`는 `@next/next/no-img-element` 경고 1개 추가(기존 `Emblem`/`ChampTile`과 동일한 근거로 이미 2건 허용된 규칙, 새 카테고리 아님). **브라우저 실측**: 새 T1 커리어를 실제로 생성해 선발 5인(Doran/Oner/Faker/Gumayusi/Keria) 전원이 role에 맞는 실명·사진으로 정확히 렌더링됨을 확인, Faker 상세 다이얼로그에서 사진·"게임 내 평가" 라벨 확인, 사진 없는 벤치 선수(합성 이름 "Rook")를 열어 이니셜 모노그램만 깨끗이 표시되고 `<img>` 자체가 아예 렌더링 안 됨(깨진 이미지 없음)을 DOM으로 직접 확인, 콘솔 에러 없음. 팀 로고 4개(Gen.G/HLE/Dplus KIA/DRX)가 실제 이미지로 렌더링됨을 확인.

### 대안(기각)
- **검색 요약 단서를 그대로 코드에 반영.** 사용자가 명시적으로 금지("검증 완료로 표시하지 마라")한 행위와 정확히 일치해 기각.
- **Liquipedia 우회 없이 사진을 포기.** Wikimedia Commons라는 대체 경로가 실제로 동작해서, 완전히 포기하는 대신 확보 가능한 만큼만(10명·4팀) 실제로 적용하는 쪽을 택함.
- **표본이 없어도 "그럴듯한" 실제 경기 기반 능력치를 만들어 낸다.** F27에서 이미 확립한 원칙("설계값을 실측으로 이름만 바꾸지 마라")과 이번 사용자 지시(§7 "자료가 없는 값에 임의로 높은 점수를 주지 마")를 동시에 위반해 명확히 기각.

관련: F16(g.scout 패턴 — 이번에도 "저장하지 않고 렌더 시점에 조회"라는 동일 원칙을 사진에 적용) / F27·D032(설계값과 실측을 분리하고 표본 부족을 정직하게 표시하는 동일 원칙) / composition.ts(F05/F06, "설계값이지 실측 아니다" 라벨링 선례 — 이번엔 선수 능력치에도 같은 라벨을 붙임).

## D035 — OP.GG로 로스터 재검증·능력치 실제 연결: 감으로 "고쳤다"고 하지 않고 브라우저로 직접 대조했다

2026-09-17. D034 이후 사용자 요청: "현재 게임에 실제 팀·소속 선수·프로필 사진·게임용 능력치를 적용해줘"(D034와 같은 전체 스펙 재확인) + "op.gg나 비슷한 사이트에서 선수정보를 가지고 수치화 해줘". 상세는 `REAL_ROSTER_AUDIT.md` §9, `PLAYER_RATING_MODEL.md` §8, `DATA_UPDATE_GUIDE.md`.

### 문제
D034 세션(2026-09-16) 후속에서 Oracle's Elixir CSV(`curl`로 접근 성공)를 실제 경기 기록 출처로 찾아 LCK_ROSTER를 갱신하고 능력치 델타(`lib/players/lck-2026-ratings.ts`)까지 계산해 뒀지만, 델타를 `lib/game.ts`에 실제로 연결(가산)하지 않은 채 세션이 끊겼다. 이번 세션 시작 시점에 그 상태를 감사하면서, CSV 기반 "핸들별 가장 최근 행" 추출 로직이 시즌 중 이적한 선수 몇 명에서 실제로는 더 이전 팀을 골라 **KT Rolster의 ADC/SUP, Kiwoom DRX의 TOP/ADC/SUP, Nongshim/BNK FearX의 ADC, 로마자 실명 여러 건**이 틀려 있다는 걸 발견했다(§9 표 참조) — CSV 자체가 아니라 매핑 스크립트의 실수였다.

### 선택
1. **op.gg 접근성을 먼저 직접 테스트했다.** WebFetch로 `esports.op.gg`를 열어보니 SPA라 빈 콘텐츠만 돌아왔다(과거 Liquipedia처럼 403/429로 막힌 게 아니라 렌더링 문제) — WebFetch를 포기하는 대신 Claude-in-Chrome 브라우저 자동화로 실제 렌더된 "선수 통계" 탭을 읽었다. 포지션 필터(탑/정글/미드/원딜/서포터)를 하나씩 눌러야 역할-핸들 매핑이 텍스트로 잡힌다는 것도 이 과정에서 확인.
2. **10개 팀 전원을 이 실측 데이터로 교차 검증**했고, 위 문제에서 발견한 6개 자리를 실제로 정정했다(§9 표) — "더 최근에 게시됐다"는 이유가 아니라, 이번 데이터가 2026-09-14까지의 실제 최근 출전 기록이라는 이유로 채택(사용자 지시 §2 "실제 적용 날짜와 등록 상태를 확인" 그대로).
3. **팀 트라이코드도 정정**했다: `orl` DRX→KRX, `wlv` KDF→DNS, `ark` FOX→BFX — op.gg가 로고 alt 텍스트·순위표에 직접 노출하는 값이라 CSV(tricode 컬럼 없음)보다 신뢰도가 높음.
4. **벤치(로테이션) 개념을 처음 실제로 사용**했다. KT SUP(Ghost/Pollu)·DNS SUP(Peter/Life)이 비슷한 출전 비중이라, 어느 한쪽을 확정 주전으로 단정하지 않고 게임 수가 많은 쪽만 `starter`, 나머지를 `bench`로 기록 — `RosterStatus`에 원래 있었지만 한 번도 안 쓰였던 값.
5. **실명은 로마자 표기까지만 저장하고 한글을 추정하지 않았다.** op.gg는 실명 로마자만 제공(한글 컬럼 없음) — 성(姓)은 로마자화가 사실상 1:1이라 기존 한글과 대조해 일치하면 그대로 유지, 불일치하면(예: Siwoo "송"→실제 "전") 오류로 판단해 로마자로 정정. 이름(given name)은 여러 한글 표기가 가능해 추정하지 않고 로마자 그대로 저장(`ROMAN_ONLY` 주석).
6. **능력치 델타를 실제로 연결했다.** `lib/game.ts`의 `newGame()`에서 `LCK_ROSTER` 실명 오버레이와 같은 지점에 `lckStatDelta(handle)`을 조회해 `makePlayer()`가 만든 합성 스탯에 **가산**(대체 아님, clamp 15~96)하도록 수정. 위 6개 자리 정정에 맞춰 델타 계산 스크립트의 대상 핸들도 갱신해 재계산(원래 빠져 있던 Rich·Andil·Ghost·Pyosik·Pollu·Life 6명분도 CSV에서 새로 계산).
7. **사진은 op.gg CDN을 핫링크하지 않기로 했다.** op.gg 선수 사진 URL(`s-qwer.op.gg/.../players/*.png`)을 DOM에서 실제로 확인했지만, Wikimedia Commons와 달리 명시적 재배포 라이선스가 없어 "사용 조건이 불명확한 이미지는 무단 재호스팅하지 말고 표시 후 기본 이미지로 대체"(사용자 지시 §4)에 따라 적용하지 않았다 — 대신 `REAL_ROSTER_AUDIT.md`에 확인한 URL 패턴과 판단 근거를 남겼다.

### 이유
"실제로 열어 확인"을 강조한 사용자 지시는 이번에도 그대로 적용된다 — CSV 기반 매핑이 계산상으로는 성공했어도, 그게 **실제로 그 팀이 지금 쓰는 선수**인지 다른 독립 출처로 대조하지 않으면 "확인"이 아니라 "계산"일 뿐이다. op.gg가 실제로 렌더링까지 해야 하는 SPA라는 이유로 WebFetch 결과(빈 콘텐츠)를 "접근 불가"로 오판하지 않고 브라우저 도구로 한 단계 더 파고든 것, 그리고 그 과정에서 자기 자신의 이전 패스(§8)가 틀렸다는 걸 발견하고 수정한 것 — 이게 이번 결정의 핵심이다. "이미 한 번 실제 데이터로 채웠다"는 사실이 다시 검증할 필요가 없다는 뜻은 아니다.

### 영향
- 변경: `lib/rosters.ts`(LCK_ROSTER 전면 재작성 — 값 대부분은 §8과 동일, 6자리만 실제로 다름, TEAM_META 트라이코드 3건), `lib/game.ts`(`lckStatDelta` 가산 로직 추가, import 1건), `lib/players/lck-2026-ratings.ts`(52개 항목으로 재계산 — §8 대비 6명 교체), `tests/real-roster.test.mjs`(§9 정정을 잠그는 회귀 테스트 2섹션 추가, 델타 클램프 검증), `REAL_ROSTER_AUDIT.md`/`PLAYER_RATING_MODEL.md`/`DATA_UPDATE_GUIDE.md`(§9 및 관련 절 추가).
- 삭제: `scratch-compute-ratings.mjs`(재현 절차를 `DATA_UPDATE_GUIDE.md`에 문서화한 뒤 삭제 — 계산 결과는 `lck-2026-ratings.ts`에 남음).
- 회귀 위험 검토: `newGame()` 핵심 경로 재변경 — tsc 0 에러, `npm run build` PASS, eslint 델타 0(`git stash` 전후 동일 4건, 전부 무관한 기존 이슈), `tests/management.test.mjs`·`tests/engine.test.mjs`(3커리어 4시즌·1095세트) PASS. **브라우저 실측**: 새 커리어를 KT Rolster로 생성해 ADC=Aiming(김하람)·SUP=Ghost(Jang Yong-jun, 로마자 실명 표시)가 실제로 렌더링됨을 확인, 선수 상세 다이얼로그에서 능력치 6축과 "게임 내 평가" 라벨이 정상 표시됨을 확인, 리그 순위표에서 KRX/DNS/BFX 트라이코드가 전부 정상 렌더링됨을 확인.

### 대안(기각)
- **CSV 매핑을 신뢰하고 op.gg 교차검증 없이 델타만 연결.** "계산이 성공했다"와 "실제로 맞다"를 구분하지 않는 셈이라 기각 — 실제로 이 검증 과정에서 6건의 진짜 오류가 나왔다.
- **op.gg 선수 사진을 그대로 핫링크.** 재배포 조건 불명확 상태에서 외부 이미지를 거는 건 사용자가 §4에서 명시적으로 경계한 행위라 기각.
- **Frog·Minous·Sharvel을 그대로 로스터에 유지.** 확인이 안 된다고 "예전 데이터가 맞겠지"로 남겨두는 것도 추정이다 — 제외하고 검토 목록으로 옮기는 쪽을 택함.

관련: D034(같은 요청의 1차 패스, 이번 패스가 그 위에서 발견한 오류를 수정) / F27·D032(실측·설계값 분리 원칙 재사용) / `PLAYER_RATING_MODEL.md` §8(3계층 설계 대신 택한 더 단순한 델타 가산 구조의 근거).
