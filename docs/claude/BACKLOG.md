...
| MATCH-SYS | P1 | IN_PROGRESS | ABIL-01·COMP-01·CAST-01을 하나의 경기 시스템으로 통합. 단위 1(전투 골격+탑/미드 갱킹, D010)·2(바텀 2v2, D011)·3(오브전 합류자 + wa 재정의, D012)·4(참여자 기반 5v5 한타, D014) 완료. 다음: 단위 5(스노볼 + 구조물 종료). 아래 "MATCH-SYS" 절 참조 |
...
### 단위 5 — 스노볼 + 구조물 기반 종료 ← 다음
**정확한 첫 작업**: `resolveTeamfight`의 결과를 구조물 진행으로 잇는다.
1. `MatchState`에 구조물 상태 추가: `towers:{A:number,B:number}`(각 팀이 잃은 포탑 수, 0~9 또는 라인별 배열), `inhibs:{A,B}`, `nexusOpen:{A,B}`. `newMatchState`에서 초기화. 저장 형식은 combat 내부 상태라 `SetResult`에 안 나가면 무영향 — 나간다면 선택 필드.
2. 한타 `DECISIVE` 직후 승자가 **생존 인원 ≥ N(예: 3) & 상대 리스폰 대기 중**이면 그 방향 라인의 구조물 1~2개 파괴(`resolveTeamfight` 반환에 `siege:{side, lane, structures:number}` 또는 별도 `resolveSiege` 호출). `ONE_SIDED`는 더 크게. 무승부·무교전은 구조물 진행 없음.
3. **종료 조건 교체**: 현재 `pressureA/B===3 || i===8`(pressure 다수) → `nexusOpen && 다음 한타 승리` 또는 누적 구조물. `i===8`까지도 안 끝나면 명시적 강제 종료(구조물 많은 쪽, 그다음 pressure, 그다음 마지막 사건 방향) — 무한 경기 방지, 테스트로 그 경로 구분.
4. 스노볼: 한타·오브·구조물 결과 → 개인 성장(`Combatant`에 누적 골드/레벨 근사) → 다음 `resolveTeamfight`의 `eff()` 또는 `numAdvFav`에 소량 반영. **D007 자원 채널(`resPow`)과 이중 보상 재점검** — 성장이 `p`와 combat 양쪽을 올리지 않게.
5. `narration.ts`: "억제기 파괴 → 넥서스 앞" 흐름. `replay.ts`: 구조물 파괴 beat + 미니맵 포탑 아이콘 상태.
6. 검증: 모든 경기 유한 종료(고정 시드 스위트), 역전이 성장 선수 사망·목표 교환에서 나오는지(강팀 은닉 약화 아님), balance-review + 통제 조건 재측정.
- 역전은 성장한 선수 사망·불리한 교전 선택·목표 교환에서 발생(강팀 은닉 약화 금지).
