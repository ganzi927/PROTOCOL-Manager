...
// 이번 범위: 지도·경로 골격 + 바텀 갱킹(resolveBotLane, 사건 index 2) 완전 연결.
// 탑·미드 갱킹(0·1)·오브전(3·4)은 위치·처치·합류/불참까지 표시하되 접근 애니메이션은 정글만 간단히.
// 한타(5~8)는 combat 데이터가 없어 팀 단위 표시(개별 피해자 스컬 없음) — 단위 4에서 상세화.
...
// 사건이 벌어지는 노드. 라인 갱킹은 지는 쪽 라인, 오브는 해당 둥지.
function eventNode(seq:number,combat:any):string{
 if(!combat||combat.kind==='teamfight'){ return seq===5?'dragon':seq===6?'baron':seq===7?'mid':'B_base'; } // 한타
...
// ── 재생 데이터 타입 ─────────────────────────────────────────────────────────
export type BeatKind='approach'|'engage'|'kill'|'secure'|'escape'|'noshow'|'revive'|'line';
...
export function buildReplay(set:{events:any[],lineupA:string[],lineupB:string[]}):ReplayData{
 ...
 // 엔진 시각 ↔ 재생 시각 매핑(revive 배치에 사용): [engineClock, tAtResolve]
 const clockMap:[number,number][]=[];
 let cursor=0;
 let scoreA=0,scoreB=0;
 let lastClock=0; // 마지막으로 아는 엔진 시각(combat 없는 한타 사건 보간용)
 const deadUntil:Record<string,number>={}; // "side slot" → 현재 부활 엔진시각(상태 판정용)
 const deaths:{rid:string, deathPlayT:number, reviveEngine:number, deathPos:Vec}[]=[]; // 사망마다 1건(부활 연출용)
 ...
 for(let ei=0;ei<events.length;ei++){
  ...
  clockMap.push([engineClock,resolveT]);
  ...
  for(const p of parts){
   ...
   // 직전 사건에서 죽었던 참가자는 이번 사건 시점엔 부활해 홈에서 출발한다(부활 걷기 키프레임은 이 루프 뒤에 삽입되므로 여기선 사망 위치가 마지막).
  ...
  for(const k of kills){
   ...
   // 피해자: 사건 위치에서 사망 상태 고정, 부활은 엔진 시각으로(사망마다 1건 기록)
   ...
   const reng=respawnEngine(engineClock);
   deadUntil[rid]=reng;
   deaths.push({rid,deathPlayT:kt,reviveEngine:reng,deathPos:dPos});
   ki++;
  }
 ...
 // 부활: 사망마다 1건(테스트4가 처치_n ↔ 부활_n을 짝지을 수 있게). 엔진 시각 → 재생 시각.
 // 부활 순간 회색 아이콘이 베이스에 나타난다. 라인 복귀 걷기는 그리지 않는다 —
 // 다음 사건 접근 루프가 홈에서 출발하므로(사망 마지막 키프레임 → homeNode) 걷기를 넣으면
 // 접근 키프레임과 시간이 겹쳐 순간이동처럼 보인다.
 for(const d of deaths){
  const [sd,slRaw]=d.rid.split(' ');const side=sd as Side, slot=Number(slRaw) as Slot;
  const rt=Math.max(d.deathPlayT+0.6, engineToPlay(clockMap,d.reviveEngine,duration));
  const baseN=side==='A'?'A_base':'B_base';
  beats.push({t:Math.min(rt,duration),seq:-1,engineClock:d.reviveEngine,kind:'revive',ref:{side,slot},text:'부활'});
  if(rt>=duration)continue;
  push({side,slot},{t:rt,pos:N[baseN].slice() as Vec,state:'dead'}); // 분수대 복귀(아직 회색, 다음 접근에서 이동 시작)
 }
...
function engineToPlay(map:[number,number][],eng:number,duration:number):number{
 if(!map.length)return duration;
 if(eng<=map[0][0])return map[0][1];
 for(let i=1;i<map.length;i++){
  if(eng<=map[i][0]){
   const [e0,t0]=map[i-1],[e1,t1]=map[i];
   return t0+(t1-t0)*((eng-e0)/((e1-e0)||1));
  }
 }
 // 마지막 사건 이후: 마지막 구간 기울기로 외삽하되 duration에서 클램프
 const [e0,t0]=map[map.length-2]??map[0],[e1,t1]=map[map.length-1];
 const slope=(t1-t0)/((e1-e0)||1);
 return Math.min(duration, t1+(eng-e1)*slope);
}
...
export function stateAt(rd:ReplayData,t:number){
 ...
 for(const bt of rd.beats){
  ...
  if(['kill','secure','revive','noshow','escape'].includes(bt.kind)) feed.push({t:bt.t,text:bt.text??bt.kind,side:bt.side,ref:bt.ref,by:bt.by});
 }
 // 골드 보간
 let ga=2500,gb=2500;
 ...
 return {score:[a,b] as [number,number], gold:[Math.round(ga),Math.round(gb)] as [number,number],
  dead:deadNow, feed:feed.slice(-5), line, seq, engineClock:win?win.engineClock:0, window:win};
}
