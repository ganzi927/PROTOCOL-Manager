...
- ⬜ 2단계: **접근 도착 시각 ↔ 합류 판정 통합**(별도 체크포인트), 한타 접근/보호 타이밍 다듬기, `notJoined` 연출, 모바일/탭전환 육안 확인, 비교 영상
...
- **`top↔river` WALK 이동 비율 2.2**(D018): WALK 그래프에 강↔탑 직결 통로 없어 우회. 다른 지역쌍은 0.66~1.45(중앙값 1.23). `replay.test` 섹션 10에서 명시적 예외.
