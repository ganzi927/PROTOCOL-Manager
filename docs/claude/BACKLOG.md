...
| MINIMAP-01/02 | **P0(사용자 재우선순위)** | IN_PROGRESS | 이동형 미니맵. 1단계(D013)·지속형 에이전트(D016)·엔진 REGION_DIST 이동 정합+구조물 SVG(D018)·사건-경계 이동 연속성(D020)·2단계 1부(D021: 합류 이동 원인 분류, 도착 기반 어시스트 표시 필터) 완료, PC 브라우저 재검증함. 다음: **엔진 참가 판정에 이동 시간 실제 통합**(범위 밖으로 명시 미룸, 수천 시드 통계 검증 필요), 모바일/탭전환 실기기 확인. 아래 "MINIMAP-01" 절 참조 |
...
### 남은 작업
- **엔진 참가 판정에 이동 시간 실제 통합**(사용자 지시의 핵심, D021에서 범위 밖으로 명시 미룸) ← **다음 첫 행동**: `resolveObjective`의 `arriveP`·`resolveTeamfight`의 `numAdvFav`가 이동 시간 기반 도착 가능성을 입력으로 받게 한다 — 기존 확률과 새 판정이 같은 실패 확률을 중복 적용하지 않게. **완료 조건**: (a) 설계를 DECISIONS 초안으로 먼저 쓰기, (b) 어떤 참가자가 새로 불참 처리되는지 표로 기록, (c) 시각화 변경과 결과-영향 변경을 별도 커밋으로 분리, (d) `teamfight-review.mjs`로 세트 승률·오브 확보율·NEXUS/CAP 비율을 D017/D019/D020과 나란히 재측정(수천 시드, McNemar CI — D014~D017급 신뢰구간 없이는 "완료" 표시 금지), (e) PC+모바일 브라우저로 확인.
- **오브젝트(전령/드래곤) 어시스트 필터 확장 확인**: 공통 코드 경로라 자동 적용되지만, `arriveP`가 이동 시간과 무관해 오브젝트 사건의 화면-판정 괴리가 더 클 수 있다 — 별도 표본 확인.
- **killer 표시 타이밍**(18.5% 불일치, D021 발견): 필터할지 kill beat 시각 자체를 늦출지 설계 먼저(골드 그래프 동기 확인 필요).
- 한타/오브/공성 사건의 진입·보호·후퇴·철수 애니메이션 타이밍을 갱킹 수준으로 다듬기, `notJoined`(리스폰 대기) 별도 연출.
- **모바일 지도 비율·아이콘 클릭→선수 상세·탭 비활성 자동 일시정지 실기기 확인**: 3연속 세션(D018·D020·D021) 모두 `resize_window`+새로고침 후 CDP 스크린샷이 1568px 고정이라 브라우저 자동화로는 확인 불가. 실제 모바일 기기 또는 브라우저 개발자 도구를 직접 조작할 수 있는 사람이 확인 필요 — 될 때까지 "완료"로 표시하지 않는다.
- 비교 영상(변경 전 = git tag `minimap-pre-persistent` 체크아웃 필요). D020·D021 둘 다 GIF 녹화까지는 했으나 파일시스템 접근 불가로 미첨부.
