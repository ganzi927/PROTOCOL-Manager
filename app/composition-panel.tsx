...
export function CompositionPanel({mine,enemy,label='우리 팀'}:{mine:string[],enemy:string[],label?:string}){
 ...
 return <section className="composition-panel" aria-label="조합 분석">
  ...
  <div className="comp-traits">{Object.entries(p.scores).map(([k,v])=><span key={k}>{({poke:'견제',engage:'진입',protect:'보호',scale:'성장'} as Record<string,string>)[k]} <b>{v.toFixed(1)}</b></span>)}</div>
