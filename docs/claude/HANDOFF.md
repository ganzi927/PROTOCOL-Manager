> 2026-09-16 최신(11): **F27(실제 조합 통계와 메타 데이터 연결) 완료 — Development Orders F01~F27 전체를 이번 세션에서 한 번씩 다뤘다.** 상세는 `BACKLOG.md`의 "F27" 절과 `DECISIONS.md`의 **D032**를 먼저 읽을 것. 요지: 명령 자체가 "외부 서비스를 화면에서 흉내 내기 전에 사용할 수 있는 데이터·접근 방식·출처·조건을 확인하라"고 요구해, 구현 전에 이 게임에 연결 가능한 외부 데이터가 실제로 있는지부터 확인했다 — **없었다**. 이 게임은 140종 전부 가상 챔피언·가상 팀으로 된 완전 자체 완결 시뮬레이션이라 실제 롤 승률 사이트가 대응할 대상 자체가 없다. `lib/balance/composition.ts`/`composition-panel.tsx`를 감사한 결과 "태그 기반 설계값을 실측 승률로 이름만 바꾸는" 위반(명령이 명시적으로 금지한 것)도 이미 없었다 — 두 파일 모두 스스로 "실측 아님"을 이미 라벨링하고 있었다. **"실제 데이터"를 이 게임 자신의 완료된 경기 기록으로 재정의**: `g.scout`(F16)가 이미 매 경기(사용자·AI 자동 진행 전부) 양 팀 관점으로 실제 픽·승패를 저장하고 있다는 걸 활용 — 새 저장소 없이 순수 읽기 전용 `lib/balance/observed.ts` 신설(`observedChampStat`/`observedTeamStats`). 표본 임계값(`MIN_SAMPLE=6`)은 3시즌 scratch 실측(관측 챔피언-역할 조합 121개 중 80%가 6표본 이상)으로 정함(감으로 안 정함). **5인 완전 조합·2인 시너지 집계는 의도적으로 안 만들었다** — 이 표본 규모에서 완전 동일 5인 조합이 반복될 확률은 사실상 0이라, "희소한 5인 조합은 샘플 부족으로 표시"를 조건문이 아니라 그 집계 함수 자체의 부재로 구조화했다(F26에서 쓴 패턴 재사용). `CompositionPanel`에 "관측 승률(이 리그 실제 경기 기록)" 절을 신설(표본 부족이면 숫자 대신 "표본 부족" 표시, 지어내지 않음), 기존 설계값 절은 "모델 조합 전망(게임 설계 지표, 실측 승률 아님)"으로 라벨 명확화 — "우리 팀 전망"(로스터 전력 카드)은 이미 별도 화면 요소라 안 건드림, 이걸로 완료 조건의 세 값이 전부 분리됨. "대회/일반 경기 맥락"을 위해 `SetScout`에 `stage?:'REGULAR'|'PLAYOFF'|'INTERNATIONAL'` 선택 필드만 추가(구세이브 무영향). "패치" 분리는 이 게임에 실시간 밸런스 패치가 없다는 걸 그대로 인정하고 `RULE_VERSION`/시즌 스탬프만 남겨둠(없는 걸 있는 척 안 함). `tests/observed-stats.test.mjs` 신설(5섹션 — 새 게임 전부 표본 부족, g.scout 재집계와 winRate 바이트 일치, 순수 함수(무변경), composition.ts와 완전 분리, 5인 조합 집계 함수 실제 미존재), 첫 실행 PASS. tsc·`npm run build`·eslint(변경 전후 동일 18 problems, 새 이슈 0개)·`management.test.mjs`·`scout.test.mjs`·`engine.test.mjs` 전부 재확인 PASS. **브라우저 실측**: 진행 중인 실제 T1 커리어에서 드래프트로 두 챔피언을 픽해 "표본 부족 (2전)"/"표본 부족 (0전)"이 정확히 렌더링됨을 확인(이 세이브는 시즌 초반이라 MIN_SAMPLE 도달 챔피언이 아직 없음 — 충분 표본 시 퍼센트 경로는 자동화 테스트가 이미 교차 검증), 콘솔 에러 없음. **F19~F27 전부 커밋 후 `git push` 완료**(사용자 지시 — 하나 끝나면 바로 push). **알려진 제한**: 5인 조합·2인 시너지·상대 조합 대응 관측 없음(v1 의도적 범위 밖), `stage` 필드는 적립만 하고 아직 어디서도 안 읽음(플레이오프만 따로 보기 등은 다음 조각), "패치"별 분리는 형식만 있고 실제로 여러 세대가 섞이는 상황 자체가 아직 없음, 표본이 전반적으로 적음(싱글플레이 커리어라 총 경기 수 제한 — 버그 아니라 실제 데이터 규모를 정직하게 반영). **다음 세션 첫 행동**: Development Orders의 F01~F27을 전부 한 번씩 다뤘으므로, 각 항목의 "알려진 제한" 절에 남은 잔여 조각(F25 첫 경기 튜토리얼이 가장 명확한 미완료 항목) 중 무엇을 먼저 할지, 또는 F15급 대규모 밸런스 검증으로 돌아갈지 사용자와 확인할 것 — 이 시점부터는 "다음 순번 F-item"이 없으니 무작정 진행하지 말 것. 완료 항목(F27) 재구현 금지.
>
> 2026-09-16 최신(10): **F26(리플레이·공유·관전 기록) 완료.** 상세는 `BACKLOG.md`의 "F26" 절과 `DECISIONS.md`의 **D031**을 먼저 읽을 것. 요지: `g.history`가 스코어라인만 남기고 전체 `SetResult`(이벤트·KDA·드래프트)는 매치가 `MATCH_END`를 지나면 버려지고 있었다. DB는 슬롯당 JSON 하나(`careers` 테이블)뿐이라 새 테이블에는 운영 DB 마이그레이션이 필요해 사용자 승인 없이는 배제 — 대신 `Game.replays?:ReplayEntry[]`를 세이브 blob 안에 직접 저장(F16 `g.scout`/F17 `g.trainingLog`와 같은 캡드 배열 패턴, `REPLAY_CAP=8`). **`finishMatch()`를 감사하다 사용자 본인 경기(`case 'continue'`)뿐 아니라 매 라운드 `autoMatch()`의 리그 전체 AI-vs-AI 경기에도 호출된다는 걸 발견** — 가드 없이 넣었으면 매 라운드 9경기어치가 전부 쌓여 무제한 증가하는 버그가 조용히 들어갈 뻔했다, `if(m.a===g.teamId||m.b===g.teamId)`로 사용자 팀 경기만 적립되게 막음. 저장 시 `ENGINE_VERSION=1`/`RULE_VERSION='classic-v1'`을 각 항목에 고정 스탬프 — F04가 설계만 하고 실구현한 적 없던 `simulationVersion` 개념을 이번에 처음 만들었다(grep으로 미구현 확인). 재계산 코드 경로 자체가 없어 "예전 리플레이는 엔진이 바뀌어도 안 바뀐다"가 조건문이 아니라 구조로 보장된다(F25의 D030과 같은 논리). "주요 장면" 북마크는 새 tier 판정을 만들지 않고 narration.ts가 이미 계산해 둔 `GameEvent.tier`를 `lib/simulation/replay.ts`의 `ReplayWindow.tier`로 그대로 옮겨 재사용, `ReplayTheater`엔 `initialSeek?:number` prop만 추가(마운트 시 1회 `seek()`, 다른 순간을 보려면 부모가 `key`를 바꿔 리마운트 — RECAP 화면이 이미 쓰던 패턴). 내보내기는 `{format,exportedAt,replay}` 하나만 담아 계정 식별자·세이브 정보가 전혀 없음을 `URL.createObjectURL`을 몽키패치해 실제 페이로드를 캡처하는 방식으로 직접 검증했다. 가져오기는 100% 클라이언트 전용(서버 미접촉)이며 보관함에 저장하지 않고 그 자리에서만 보여준다. `tests/replay-archive.test.mjs` 신설(4섹션 — 사용자 팀 경기만 적립, 배경 리그 경기 미적립, CAP 적용, 이후 진행과 무관하게 저장 항목 불변, tier 반영), 첫 실행 PASS. tsc·`npm run build`·eslint(변경 전후 동일 18 errors/15 warnings — 새로 만든 에러 0개 직접 대조 확인)·`management.test.mjs`·`engine.test.mjs`(3커리어·1106+ 세트) 전부 재확인 PASS. **브라우저로 실제 Bo3 매치 2세트를 드래프트부터 매치 결산까지 전부 플레이해 검증**: `g.replays`에 정확한 엔진/규칙 버전으로 2항목 적립 확인 → 보관함 목록·상세(미니맵·KDA·주요 장면 칩 5개) 정상 렌더 → 칩 클릭 시 정확한 타임코드(29:23)로 즉시 이동 → 내보내기 페이로드가 리플레이 단 하나뿐임을 직접 캡처해 확인 → 그 파일을 "공유받은 리플레이 보기"로 재주입해 원본과 동일하게 렌더링됨을 확인. 콘솔 에러 없음. **F19~F26 전부 커밋 후 `git push` 완료**(사용자 지시 — 하나 끝나면 바로 push). **알려진 제한**: 영상(비디오) 내보내기 없음(완료 조건이 명시한 대로 재생 안정화 이후로 미룸), `ENGINE_VERSION`은 상수 1 고정이라 실제 엔진이 바뀔 때 값을 올리는 절차가 아직 없음(사람이 기억해서 올려야 함), 사용자 팀이 아닌 경기의 관전 리플레이는 범위 밖. **다음 세션 첫 행동**: F27(실제 조합 통계와 메타 데이터 연결)로 순차 진행하거나, F25의 미완료 잔여 과제(첫 경기 튜토리얼)로 돌아갈 것 — 착수 전 이 파일의 F19~F26 "알려진 제한" 문단부터 읽을 것. 완료 항목(F26) 재구현 금지.
>
> 2026-09-16 최신(9): **F25(첫 경기 튜토리얼과 연습 모드) — 연습 모드만 완료, 튜토리얼은 미착수.** 상세는 `BACKLOG.md`의 "F25" 절과 `DECISIONS.md`의 **D030**을 먼저 읽을 것. 요지: `Game.sandbox?:boolean` 필드가 이미 선언돼 있었지만 코드 어디서도 안 읽는 죽은 필드였다(기능 미구현). `simulateSet`을 감사한 결과 `g.players`/`g.teams`를 읽기만 하고 아무것도 안 쓰는 순수 함수임을 확인 — 실제로 번아웃·폼·숙련도·POG·스카우팅·훈련이력·`g.history`·팀 승패를 갱신하는 건 별도 함수 `finishMatch()`뿐이라, `case 'sandboxMatch'`가 `finishMatch`를 절대 안 부르면 "본 시즌에 안 섞인다"가 조건문이 아니라 **코드 경로 부재로 구조적으로 보장**된다. 결과는 `Game.sandboxLast`(마지막 1건)에 저장되며 F24에서 확인한 기존 저장 인프라(revision/영수증/백업)를 그대로 재사용. 재현성은 시드를 클라이언트가 명시적으로 넘기는 방식(`applyCommand` 순수성 유지 — 명령 처리 중 `Math.random()` 미사용). 화면은 새로 안 만들고 `SetResult`가 이미 `ReplayTheater`가 요구하는 필드를 전부 갖고 있어 그대로 재사용 — 미니맵·중계까지 실제 경기와 동일 품질. `tests/sandbox.test.mjs` 신설(4섹션), 첫 실행 PASS. tsc/build/`management.test.mjs`/`engine.test.mjs`(1106세트) 재확인 PASS. **브라우저로 실제 매치 실행 + 재현성까지 직접 확인**: T1 vs GEN 실행 → 미니맵·중계 정상 재생 → "같은 조건 다시 보기" → 클럭·이벤트·골드가 정확히 동일하게 재생됨을 확인. **F19~F25(연습 모드) 전부 커밋 후 `git push` 완료**(사용자 지시 — 하나 끝나면 바로 push). **미완료로 정직하게 남긴 것**: 첫 경기 튜토리얼(조합 목표·밴픽 이유·감독 지시·리캡 근거를 첫 경기에서 하나씩 안내하는 오버레이) — 근거 콘텐츠 자체는 F04~F14에 이미 다 있음을 확인했지만, 여러 화면에 걸친 UI choreography라 별도 세션의 첫 작업으로 미룸. **다음 세션 첫 행동**: F25 튜토리얼 이어서 하거나 F26(리플레이·공유·관전 기록)으로 진행 — 착수 전 이 파일의 F19~F25 "알려진 제한"·"다음" 문단부터 읽을 것. 완료 항목(연습 모드) 재구현 금지.
>
> 2026-09-16 최신(8): **F24(저장 안정성과 세이브 이동) 완료.** 상세는 `BACKLOG.md`의 "F24" 절과 `DECISIONS.md`의 **D029**를 먼저 읽을 것. 요지: `app/api/game/route.ts`를 감사했더니 이미 매우 탄탄했다 — `commandId` 명령 영수증(네트워크 재시도·중복 클릭 안전), `revision` 기반 SQL `UPDATE...WHERE revision=?` 원자적 검사(DB 레벨 낙관적 동시성, "다른 기기 동시 진행"을 앱 로직이 아니라 트랜잭션으로 막음), 최근 3개 백업 로테이션, 미래 버전 거부(`upgradeGame`)까지 전부 이미 있었다 — 재구현 안 함. **유일한 진짜 공백**: 설정 화면이 스스로 "내보낸 파일의 다시 불러오기는 이 버전에서 지원하지 않습니다"라고 인정하고 있었다 — 내보내기는 있는데 가져오기가 없어 "세이브 이동"이 F24 제목 그대로 안 됐다. `applyCommand`에 `importSave` 케이스를 신설해 **새 API 경로를 만들지 않고 기존 명령 흐름에 그대로 얹었다** — 그 덕에 "원본 보존"(가져오기 직전 상태가 자동으로 복원 지점에 들어감)이 기존 백업 로테이션에서 공짜로 나온다. 형식 검사는 얕은 shape 확인, 버전 검사·마이그레이션은 전부 기존 `upgradeGame()` 재사용(새 버그 자리 없음). 요청 크기 상한을 명령 종류별로 분리(일반 10000바이트 유지, `importSave`만 1.2MB — 4시즌 실측 268KB에 여유). `tests/save-import.test.mjs` 신설(5섹션), 첫 실행 PASS. **브라우저로 전체 왕복 실측**: 내보내기 → 현금 값을 표식(123456789)으로 바꿔 가져오기 → 화면에 정확히 "12345.68억"으로 반영 확인 → "자동 복원 지점"으로 원래 값(9.29억) 정확히 복구 확인 — 사용자의 실제 저장 데이터를 안전하게 원상복구했다. tsc/build/`management.test.mjs`/`engine.test.mjs` 재확인 PASS. **F19~F24 전부 커밋 후 `git push` 완료**(사용자 지시 — 하나 끝나면 바로 push). **알려진 제한**: 빈 슬롯으로의 직접 가져오기 미지원(먼저 팀 선택해 커리어를 만들어야 함), `simulationVersion`(엔진 로직 버전, 스키마 버전과는 별개)은 F04 설계 의도에만 있었고 실제 구현된 적 없음을 확인 — 리플레이가 아직 장기 보존 아티팩트가 아니라 급하지 않다고 판단해 다음 조각(F26)으로 이관. **다음 세션**: F25(첫 경기 튜토리얼과 연습 모드)로 순차 진행 — 착수 전 이 파일의 F19~F24 "알려진 제한" 문단부터 읽을 것. 완료 항목 재구현 금지.
>
