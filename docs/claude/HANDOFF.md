> 2026-09-16 최신(7): **F23(모바일과 경기 길이 조절) 완료.** 상세는 `BACKLOG.md`의 "F23" 절과 `DECISIONS.md`의 **D028**을 먼저 읽을 것. 요지: 체크리스트 항목 대부분(프레임률-엔진 독립, 키보드 포커스, CSS `prefers-reduced-motion`, 색상 외 진영 구분, density 토글)이 이미 충족돼 있어 감사만 하고 재구현 안 함. **진짜 공백 하나 발견**: 미니맵 선수 위치가 CSS transition이 아니라 매 `requestAnimationFrame`마다 JS가 직접 `setAttribute`로 갱신해서, OS의 "모션 감소" 설정이 CSS 애니메이션만 덮고 이 JS 갱신엔 전혀 관여하지 못하고 있었다. `formationOffset`(이미 "표시 전용 오프셋"이라고 주석에 명시된 순수 함수)에 선택적 `reducedMotion` 인자 추가(기본 false, 생략 시 기존과 100% 동일 — 테스트로 직접 증명), 모션 감소 활성 시 실제 DOM 페인트 빈도만 ~150ms로 제한(엔진 시계는 매 프레임 그대로 전진 — 배속·되감기·결과 무영향). 좁은 화면(≤767px) 첫 진입 시(저장된 선호 없을 때만) 리캡 밀도 기본값을 'summary'로. `tests/broadcast.test.mjs`에 reducedMotion 검정 3개 추가, 첫 실행 PASS. tsc/build/`replay.test.mjs` 재확인 PASS. **실기기/뷰포트 렌더링은 이번에도 BLOCKED** — `resize_window` 후에도 스크린샷이 1568px 데스크톱으로 캡처됨(D018부터 5세션 연속 같은 도구 제약, 완료로 위장하지 않고 정직하게 기록). 터치 목표(`sm` 버튼 32px, WCAG AA는 충족·AAA는 아님)는 공유 컴포넌트라 감사만 하고 변경 안 함. **F19~F23 전부 커밋 후 `git push` 완료**(사용자 지시 — 하나 끝나면 바로 push). **다음 세션**: F24(저장 안정성과 세이브 이동)로 순차 진행 — 착수 전 이 파일의 F19~F23 "알려진 제한" 문단부터 읽을 것. 완료 항목 재구현 금지.
>
> 2026-09-16 최신(6): **F22(시즌 서사와 규칙 버전별 메타) 완료.** 상세는 `BACKLOG.md`의 "F22" 절과 `DECISIONS.md`의 **D027**을 먼저 읽을 것. 요지: `news()`를 먼저 감사했더니 이미 전부 실제 사건(계약·경기·스폰서) 기반이라 "무작위 뉴스" 문제 자체가 없었다 — 재구현 대상이 아니었음. 진짜 빠진 건 `g.history`/`standings()`/로스터를 종합해 서사로 뽑아 주는 기능 자체 — `seasonNarrative(g,id)` 신설(저장 안 함, 순수 함수): 연승/연패(최신 기록부터 3연속 이상만), 2번 이상 맞붙은 상대 중 가장 팽팽한 라이벌전, 벤치가 −2 OVR 이내로 근접한 주전 경쟁, 6위 컷과 1승 이내인 플레이오프 경쟁을 감지. "신인 성장"은 시즌 시작 스냅샷이 없어 거짓 근거를 만들지 않으려고 정직하게 뺐다(다음 조각). "규칙 버전별 메타"도 조사해 보니 기존 `g.meta`(시즌마다 재계산되는 챔피언 목록)가 이미 "패치"에 해당하는 시스템이었고, 진행 중 경기 불변·리플레이 보존(재생 렌더러가 `isMeta`를 아예 호출 안 함, 시뮬레이션 시점에만 baked-in)·정직한 라벨링을 전부 이미 충족하고 있었다 — 유일한 진짜 공백(변경 시점이 어디에도 안 남음)만 `nextYear()`에 news 한 줄로 보완. 감독실 홈에 "이번 시즌 이야기" 패널 신설(빈 배열이면 조용히 안 뜸 — 항상 뭔가 보여주는 가짜 패널 아님). `tests/narrative.test.mjs` 신설(5섹션), 첫 실행 PASS. `nextYear()` 변경 폭이 news 문자열 1줄뿐이라 회귀 위험 낮음 — tsc/build/`management.test.mjs`/`engine.test.mjs` 전부 재확인 PASS. 브라우저 확인: 실제 저장(최근 경기 패배라 연승 조건 미충족)에서 패널이 정상적으로 안 뜨는 것까지 확인, 콘솔 에러 없음. **F19~F22 전부 커밋 후 `git push` 완료**(사용자 지시 — 하나 끝나면 바로 push). **알려진 제한**: "신인 성장" 스토리라인 없음(새 필드 필요), 진짜 게임-규칙 패치 시스템(오브젝트·시야 공식 변경 등)은 안 만듦(기존 메타 로테이션의 고지 공백만 보완), 스토리라인 감지 문턱은 설계 가설(대규모 검증 전). **다음 세션**: F23(모바일과 경기 길이 조절)으로 순차 진행 — 착수 전 이 파일의 F19~F22 "알려진 제한" 문단부터 읽을 것. 완료 항목 재구현 금지.
>
